       ...
       m_timerIdleWakeUp(this), m_queue(NULL), m_queueLength(0), m_checkReturnCode(true)
{
    ...
    wxFont font(wxDEFAULT, wxFONTFAMILY_TELETYPE, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL);
    if ( font.Ok() ) {
...
void MyExecPanel::OnProcessTerminated(MyPipedProcess *process, int pid, int status)
{
    ...
    {
        // add elapsed time
        const long duration = hugin_utils::roundi(m_stopWatch.Time() / 1000.0);
        if (duration >= 60)
        {
            std::ldiv_t result=std::ldiv(duration, 60l);
