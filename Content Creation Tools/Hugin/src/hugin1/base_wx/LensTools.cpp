...
{
    list->Clear();
    HuginLensTools::LensInfoVector lensInfoVector(GetLensProjectionList());
    for (auto& lensInfo : lensInfoVector)
    {
        list->Append(lensInfo.name, (void*)lensInfo.id);
    };
    list->SetSelection(0);
}
WXIMPEX HuginLensTools::LensInfoVector GetLensProjectionList()
{
    // add all lens types to vector
    HuginLensTools::LensInfo lensInfo;
    HuginLensTools::LensInfoVector lensInfoVector;;
    lensInfo.id = HuginBase::SrcPanoImage::RECTILINEAR;
    lensInfo.name = _("Normal (rectilinear)");
    lensInfoVector.push_back(lensInfo);
    lensInfo.id = HuginBase::SrcPanoImage::PANORAMIC;
    lensInfo.name = _("Panoramic (cylindrical)");
    lensInfoVector.push_back(lensInfo);
    lensInfo.id = HuginBase::SrcPanoImage::CIRCULAR_FISHEYE;
    lensInfo.name = _("Circular fisheye");
    lensInfoVector.push_back(lensInfo);
    lensInfo.id = HuginBase::SrcPanoImage::FULL_FRAME_FISHEYE;
    lensInfo.name = _("Full frame fisheye");
    lensInfoVector.push_back(lensInfo);
    lensInfo.id = HuginBase::SrcPanoImage::EQUIRECTANGULAR;
    lensInfo.name = _("Equirectangular");
    lensInfoVector.push_back(lensInfo);
    lensInfo.id = HuginBase::SrcPanoImage::FISHEYE_ORTHOGRAPHIC;
    lensInfo.name = _("Orthographic");
    lensInfoVector.push_back(lensInfo);
    lensInfo.id = HuginBase::SrcPanoImage::FISHEYE_STEREOGRAPHIC;
    lensInfo.name = _("Stereographic");
    lensInfoVector.push_back(lensInfo);
    lensInfo.id = HuginBase::SrcPanoImage::FISHEYE_EQUISOLID;
    lensInfo.name = _("Equisolid");
    lensInfoVector.push_back(lensInfo);
    lensInfo.id = HuginBase::SrcPanoImage::FISHEYE_THOBY;
    lensInfo.name = _("Fisheye Thoby");
    lensInfoVector.push_back(lensInfo);
    return lensInfoVector;
}
;
