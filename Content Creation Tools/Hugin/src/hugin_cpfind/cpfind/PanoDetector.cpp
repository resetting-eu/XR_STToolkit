...
void PanoDetector::run()
{
    ...
    if (_cores == 0)
    {
        // use number of physical cores - 1
        size_t nrCores = utils::GetPhysicalCPUCount();
        if (nrCores > 4)
        {
            --nrCores;
        };
    ...
    std::cout << "Using " << _cores << " threads" << std::endl;
#endif
...
#define getcwd _getcwd
    ...
    // create a hash set for all known control points
    {
        const HuginBase::CPVector& allCps = _panoramaInfo->getCtrlPoints();
        for (auto& cp : allCps)
        {
            _cpsHashSet.insert(cp.getCPString());
        };
    }
